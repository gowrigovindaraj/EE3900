...
\begin{abstract}
This document contains the solution to Exercise 2.27 of Oppenheim.
\end{abstract}

\problem Three systems A, B and C have the inputs and outputs as indicated in Fig. Determine whether each system could be LTI. If your answer is yes, specify whether there could be more than one LTI system with the given input-output pair. Explain your answer.
\\
\begin{figure}[!h]
\centering
...
\node[box] (a) {System A};
\node[left=of a] (aux1) {$\brak{\frac{1}{2}}^n$};
\node[right=of a] (c) {$\brak{\frac{1}{4}}^n$};
\draw[->] (aux1) -- (a);
 \draw[->] (a) -- (c);
\end{tikzpicture}
\begin{tikzpicture}[box/.style={draw,text width=2.5cm,align=center}]
\node[box] (a) {System B};
\node[left=of a] (aux1) {$e^{j\frac{n}{8}}u[n]$};
\node[right=of a] (c) {$2e^{j\frac{n}{8}}u[n]$};
\draw[->] (aux1) -- (a);
 \draw[->] (a) -- (c);
\end{tikzpicture}
\begin{tikzpicture}[box/.style={draw,text width=2.5cm,align=center}]
\node[box] (a) {System C};
\node[left=of a] (aux1) {$e^{j\frac{n}{8}}$};
\node[right=of a] (c) {$2e^{j\frac{n}{8}}$};
\draw[->] (aux1) -- (a);
 \draw[->] (a) -- (c);
\end{tikzpicture}
\end{figure}
